...
namespace hrms_be_backend_business.Logic
{
    public class LearningAndDevelopmentMailService : ILearningAndDevelopmentMailService
    {
        ...
        public async Task SendTrainingInductionApprovalMailToApprover(long ApproverUserId, long TrainingInductionByUserId, string TrainingProvider)
        {
            try
            {
                var userDetails = await _accountRepository.FindUser(ApproverUserId);
                var trainingInductionBy = await _accountRepository.FindUser(TrainingInductionByUserId);
                StringBuilder mailBody = new StringBuilder();
                mailBody.Append($"Dear {userDetails.FirstName} {userDetails.LastName} {userDetails.MiddleName} <br/> <br/>");
                mailBody.Append($"Kindly login to approve a training induction request by {trainingInductionBy.FirstName} {trainingInductionBy.LastName} {trainingInductionBy.MiddleName} <br/> <br/>");
                mailBody.Append($"<b>Training Provider : <b/> {TrainingProvider}  <br/> ");

                var mailPayload = new MailRequest
                {
                    Body = mailBody.ToString(),
                    Subject = "Training Induction",
                    ToEmail = userDetails.OfficialMail,
                };
                SendEmailAsync(mailPayload, null);
            ...
            {
                _logger.LogError(ex.ToString(), new { Controller = "LearningAndDevelopmentMailService", Method = "SendTrainingInductionApprovalMailToApprover" });
            }
        }

        public async Task SendTrainingInductionApproveConfirmationMail(long RequesterUserId, long ApprovedByUserId, string TrainingProvider)
        {
            try
            {
                var userDetails = await _accountRepository.FindUser(RequesterUserId);
                ...
                StringBuilder mailBody = new StringBuilder();
                mailBody.Append($"Dear {userDetails.FirstName} {userDetails.LastName} {userDetails.MiddleName} <br/> <br/>");
                mailBody.Append($"Your training induction has been approved by {ApprovedByUserDetails.FirstName} {ApprovedByUserDetails.LastName} {ApprovedByUserDetails.MiddleName} <br/> <br/>");
            ...
            {
                _logger.LogError(ex.ToString(), new { Controller = "LearningAndDevelopmentMailService", Method = "SendTrainingInductionApproveConfirmationMail" });
            }
        }

        public async Task SendTrainingInductionDisapproveConfirmationMail(long RequesterUserId, long DiapprovedByUserId)
        {
            try
            {
                var userDetails = await _accountRepository.FindUser(RequesterUserId);
                ...
                StringBuilder mailBody = new StringBuilder();
                mailBody.Append($"Dear {userDetails.FirstName} {userDetails.LastName} {userDetails.MiddleName} <br/> <br/>");
                mailBody.Append($"Your training induction has been disapproved by {disapprovedByUserDetails.FirstName} {disapprovedByUserDetails.LastName} {disapprovedByUserDetails.MiddleName} <br/> <br/>");
            ...
            {
                _logger.LogError(ex.ToString(), new { Controller = "LearningAndDevelopmentMailService", Method = "SendTrainingInductionDisapproveConfirmationMail" });
            }
        }
        ...
        public async Task SendTrainingScheduleNotificationMail(long StaffUserId, string TrainingOrganizer, string TrainingVenue, string TrainingMode, string TrainingTime, string TrainingTopic, string TrainingDate)
        {
            try
            {
                ...
                {
                    //Mail to Hod
                    var HodDetails = await _employeeRepository.GetEmployeeById(userDetails.HodEmployeeId, userDetails.CompanyID);
                    StringBuilder HodmailBody = new StringBuilder();
                    mailBody.Append($"Dear {HodDetails.Employee.FirstName} {HodDetails.Employee.LastName} {HodDetails.Employee.MiddleName} <br/> <br/>");
                    ...
                    var HodmailPayload = new MailRequest
                    {
                        Body = mailBody.ToString(),
                        Subject = "Training Schedule",
                        ToEmail = HodDetails.Employee.OfficialMail,
                    };
                ...
                {
                    //Mail to UnitHead
                    var unitHeadDetails = await _employeeRepository.GetEmployeeById(userDetails.UnitHeadEmployeeId, userDetails.CompanyID);
                    StringBuilder UnitHeadmailBody = new StringBuilder();
                    mailBody.Append($"Dear {unitHeadDetails.Employee.FirstName} {unitHeadDetails.Employee.LastName} {unitHeadDetails.Employee.MiddleName} <br/> <br/>");
                    mailBody.Append($"{userDetails.FirstName} {userDetails.LastName} {userDetails.MiddleName} have been Scheduled for training <br/> <br/>");
                    ...
                    var UnitHeadmailPayload = new MailRequest
                    {
                        Body = mailBody.ToString(),
                        Subject = "Training Schedule",
                        ToEmail = unitHeadDetails.Employee.OfficialMail,
                    };
